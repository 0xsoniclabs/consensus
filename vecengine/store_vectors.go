...
func (vi *Engine) getBytes(table kvdb.Store, id ctype.EventHash) []byte {
	key := id.Bytes()
	b, err := table.Get(key)
	if err != nil {
		vi.crit(err)
...
func (vi *Engine) setBytes(table kvdb.Store, id ctype.EventHash, b []byte) {
	key := id.Bytes()
	err := table.Put(key, b)
	if err != nil {
		vi.crit(err)
...
func (vi *Engine) GetLowestAfter(id ctype.EventHash) *LowestAfterSeq {
...
func (vi *Engine) GetHighestBefore(id ctype.EventHash) *HighestBeforeSeq {
...
func (vi *Engine) SetLowestAfter(id ctype.EventHash, seq *LowestAfterSeq) {
...
func (vi *Engine) SetHighestBefore(id ctype.EventHash, seq *HighestBeforeSeq) {
