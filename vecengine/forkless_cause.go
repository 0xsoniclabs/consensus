...
func (vi *Engine) ForklessCause(aID, bID ctype.EventHash) bool {
...
func (vi *Engine) forklessCause(aID, bID ctype.EventHash) bool {
	...
	for branchIDint, creatorIdx := range branchIDs {
		branchID := ctype.ValidatorIdx(branchIDint)
...
func (vi *Engine) ForklessCauseProgress(aID, bID ctype.EventHash, candidateParents, chosenParents ctype.EventHashes) (*ctype.WeightCounter, []*ctype.WeightCounter) {
	...
	candidateParentsFCProgress := make([]*ctype.WeightCounter, len(candidateParents))
...
func maxEvent(a ctype.Seq, b ctype.Seq) ctype.Seq {
	if a > b {
		return a
	}
	return b
