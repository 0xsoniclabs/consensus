...
func testRestartAndReset(t *testing.T, weights []pos.Weight, mutateWeights bool, cheatersCount int, resets bool) {
	...
	for _, e := range ordered {
		...
		if r.Intn(10) == 0 {
			...
			restored := NewIndexedLachesis(store, prev.input, &adapters.VectorToDagIndexer{Engine: vecengine.NewIndex(prev.crit, vecengine.LiteConfig())}, prev.crit, prev.config)
