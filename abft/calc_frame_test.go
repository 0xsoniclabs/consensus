...
func testCalcFrameIdx(t *testing.T, gap int) {
	nodes := consensustypes.GenNodes(2)
	// Give one validator quorum power to advance the frames on it's own
	lch, _, store, _ := NewCoreLachesis(nodes, []consensustypes.Weight{1, 3})

	laggyGenesis := processTestEvent(t, lch, store, nodes[0], 1, consensustypes.EventHashes{})
	parentEvent := processTestEvent(t, lch, store, nodes[1], 1, consensustypes.EventHashes{})
	for i := 0; i < gap; i++ {
		parentEvent = processTestEvent(t, lch, store, nodes[1], consensustypes.Seq(parentEvent.Seq()+1), consensustypes.EventHashes{parentEvent.ID()})
	}
	// Lagging validator creates an event after a frame gap
	finalEvent := processTestEvent(t, lch, store, nodes[0], laggyGenesis.Seq()+1, consensustypes.EventHashes{laggyGenesis.ID(), parentEvent.ID()})

	if want, got := laggyGenesis.Frame()+consensustypes.Frame(gap)+1, finalEvent.Frame(); want != got {
		t.Errorf("expected calculated frame number of lagging validator to be: %d, got: %d", gap, finalEvent.Frame())
	}
}

var maxLamport consensustypes.Lamport = 0

// processTestEvent builds and pipes the event through main Lacehsis' DAG manipulation pipeline
func processTestEvent(t *testing.T, lch *CoreLachesis, store *EventStore, validatorId consensustypes.ValidatorID, seq consensustypes.Seq, parents consensustypes.EventHashes) *consensustypes.TestEvent {
	event := &consensustypes.TestEvent{}
	event.SetSeq(seq)
