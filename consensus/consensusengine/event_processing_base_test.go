...
func TestLachesisClassicBases(t *testing.T) {
	testSpecialNamedBases(t, `
...
func TestLachesisRandomBases(t *testing.T) {
	// generated by codegen4LachesisRandomBase()
	testSpecialNamedBases(t, `
 A1.01    
 ║         ║        
 ╠════════ B1.01    
...
// testSpecialNamedBases is a general test of base selection.
// Event name means:
// - 1st letter uppercase - event should be base;
// - 2nd number - frame where event should be in;
// - "." - separator;
// - tail - makes name unique;
func testSpecialNamedBases(t *testing.T, scheme string) {
	t.Helper()
	//logger.SetTestMode(t)
	assertar := assert.New(t)

	// decode is a event name parser
	decode := func(name string) (frameN consensus.Frame, isBase bool) {
		n, err := strconv.ParseUint(strings.Split(name, ".")[0][1:2], 10, 64)
		if err != nil {
			panic(err.Error() + ". Name event " + name + " properly: <UpperCaseForBase><FrameN><Engine>")
		}
		frameN = consensus.Frame(n)

		isBase = name == strings.ToUpper(name)
		return
	}
...
/*
// codegen4LachesisRandomBase is for test data generation.
func codegen4LachesisRandomBase() {
	...
	for _, e := range config {
		...
		_, isBase := frame.Bases[e.Creator][e.ID()]
